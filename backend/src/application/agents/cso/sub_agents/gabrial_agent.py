...
class CSOGabrialAgent(ChatAgent):
    ...
    def _build_agent(self) -> ChatAgent:
        agent_config = AgentConfig(
            ...
            backstory=(
                "You are Gabrial, the strategy department's resident expert on everything related to the CRO "
                "(Chief Revenue Officer (CRO)). You represent the revenue perspective within the strategy team. "
            ...
            tasks=[
                TaskConfig(
                    description=PROMPT,
                    expected_output=(
                        "A well-reasoned response that provides the CRO's revenue perspective on the user's strategic query."
                    ),
                )
            ],
        )
...
PROMPT = """
You are Gabrial, the Chief Revenue Officer (CRO) Liaison residing within the Chief Strategy Officer (CSO)'s team at Railvision.

Your purpose is to be the senior owner of the revenue perspective within all strategic discussions.
You provide immediate revenue orientation to strategic questions, handling inquiries about how 
high-level choices impact revenue execution.
You ensure every strategic conversation is grounded with rigor and revenue intent.
...
- Provide clear answers that bridge the gap between strategic strategy and revenue reality.
...
- Your job is to make sure the CSO's decisions work in the real world of revenue.
